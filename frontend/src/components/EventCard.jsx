...
const EventCard = (props) => {
  ...
  const cancelBookingEvent = () => {
    cancelBooking({
      ...
      refetchQueries: [
        { query: GET_BOOKED_EVENTS, variables: { userId: userId } },
      ],
    })
  ...
  return (
    <div className="card-container">
      <h3>{event.name}</h3>
      <p id="date">
        Event date: <span>{getDate(event.date)}</span>
      </p>
      <p id="desc">{event.description}</p>
      {selected === "all" && event.creator.id === userId ? (
        <button className="secondary-btn">View Event Details</button>
      ) : selected === "booked" ? (
        <button className="secondary-btn" onClick={cancelBookingEvent}>
          Cancel Booking
        </button>
      ) : selected === "user" ? (
        ...
        <button className="secondary-btn" onClick={bookEvent}>
